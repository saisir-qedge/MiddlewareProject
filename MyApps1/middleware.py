...
class AppMaintananceMiddleware(object):
    ...
    def __call__(self, request):
        return HttpResponse(
            '<h1>Currently Application under maintenance...<br /><br />Plz try after 6am..!!</h1><hr />')
...
class ErrorMessageMiddleware(object):
    ...
    def process_exception(self, request, exception):
        ...
        return HttpResponse(
            '<h1> Currently we are facing some technical problems..(Exception) <br /><br />Plz try after some time !!!</h1><hr /><h2>Raised Exception:{}</h2><h3>Exception Message : {}</h3>'.format(exception.__class__.__name__, exception))
